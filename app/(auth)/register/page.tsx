  ...
  return (
    <div className="auth-card">
      ...
      <form onSubmit={handleSubmit} className="auth-form">
        ...
        <input type="text" className="auth-input" placeholder="Nama kamu" value={name} onChange={(e) => setName(e.target.value)} required />
        ...
        <div className="auth-input-wrap">
          <input type={showPassword ? "text" : "password"} className="auth-input" placeholder="Minimal 6 karakter" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} />
        ...
        <button type="submit" className="auth-btn-primary" disabled={loading}>{loading ? "Memproses…" : "Daftar sekarang →"}</button>
      </form>

      <p className="auth-footer">Sudah punya akun? <Link href="/login">Masuk di sini</Link></p>
    </div>
  );
}